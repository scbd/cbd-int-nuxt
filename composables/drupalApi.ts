...
export const getDrupalLanguages = async (language: string | null) => {
  const langCode =
    language && ["fr", "es", "ru", "zh-hans", "ar"].includes(language)
      ? language
      : "";
  ...
  try {
    const response = await fetch(
      `${config.public.DRUPAL_URL}/${langCode}/jsonapi/configurable_language/configurable_language`,
      {
        method: "GET",
        headers: {
    ...
    if (response.ok) {
      ...
      const languages = data.data
        .filter(
          (item: { attributes: { locked: boolean } }) =>
            item.attributes.locked === false
        )
        .map(
          (item: {
            attributes: {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.data && data.data.length > 0) {
        ...
        const rootItems = data.data.filter(
          (item: { attributes: { parent: null } }) =>
            item.attributes.parent === null
        );
        processedMenu.menu = rootItems.map((item: drupalMenu) => {
          ...
          const children = data.data.filter(
            (child: { attributes: { parent: string } }) => {
              return (
                child.attributes.parent &&
                child.attributes.parent.replace("menu_link_content:", "") ===
                  item.id
              );
            }
          );
          ...
          if (children.length > 0) {
            menuItem.children = children.map((child: drupalMenu) => {
              ...
              const grandchildren = data.data.filter(
                (grandchild: drupalMenu) => {
                  return (
                    grandchild.attributes.parent &&
                    grandchild.attributes.parent.replace(
                      "menu_link_content:",
                      ""
                    ) === child.id
                  );
              ...
              if (grandchildren.length > 0) {
                childItem.children = grandchildren.map(
                  (grandchild: drupalMenu) => {
                    return {
                      id: grandchild.id,
...
export const setLanguage = async (language: string) => {
  ...
  const langCode =
    language && ["en", "fr", "es", "ru", "zh-hans", "ar"].includes(language)
      ? language
      : "en";
